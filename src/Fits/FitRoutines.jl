...
function fit_routine_correlated(model::Function, xdata::Array{<:Real}, ydata::Array{uwreal}, param::Int64=3; pval::Bool=false, wpm::Union{Dict{Int64,Vector{Float64}},Dict{String,Vector{Float64}}, Nothing}=nothing, info::Bool=false, lineprint::Bool=true, fitRes::Bool=false, ensList::Union{Vector{String}, Nothing}=nothing)
    isnothing(wpm) ? uwerr.(ydata) : [uwerr(yaux, wpm) for yaux in ydata]
    yval = value.(ydata)
    yer = err.(ydata)
    
    lineprint ? @info("Correlated fit") : nothing
    mcov = ADerrors.cov(ydata)
    mcov_inv = inv(mcov)
    mcov_inv = (mcov_inv + mcov_inv') / 2

    chisq = gen_correlated_chisq(model, xdata, mcov_inv)
    fit = curve_fit(model, xdata, yval, mcov_inv, fill(0.5, param))
    (upar, chi_exp) = isnothing(wpm) ? fit_error(chisq, coef(fit), ydata, W=mcov_inv) : fit_error(chisq, coef(fit), ydata, wpm, W=mcov_inv)
    chi2_fit_res = sum(fit.resid.^2)
